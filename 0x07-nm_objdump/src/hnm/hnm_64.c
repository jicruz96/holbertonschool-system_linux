...
int hnm_64(char *program_name, char *file, int elf_fd, int encoding)
{
	...
	Elf64_Ehdr h;
	Elf64_Shdr section;
	uint64_t symbol_table_offset = 0, num_rows = 0;
	...
	for (i = 0; i < h.e_shnum; i++)
	{
		...
		if (encoding == ELFDATA2MSB)
			bswap_Elf64_Shdr(&section);

		if (section.sh_type == SHT_SYMTAB)
		{
			/* We'll skip the first symbol, which is always empty */
			symbol_table_offset = section.sh_offset + sizeof(Elf64_Sym);
			num_rows = (section.sh_size / sizeof(Elf64_Sym)) - 1;
			break;
		}
	}
	...
	print_symbol_table_64(elf_fd, encoding,
...
/**
 * print_symbol_table_64 - prints a symbol table for a 64-bit ELF file
 * @elf_fd: ELF file descriptor
	...
	uint64_t symbol_table_offset, uint64_t num_rows)
{
	...
	if (encoding == ELFDATA2MSB)
		bswap_Elf64_Shdr(&string_table_Shdr);
	...
	for (i = 0; i < num_rows; i++)
		print_symbol_table_row_64(elf_fd, encoding,
...
/**
 * print_symbol_table_row_64 - prints a row of a 64-bit ELF symbol table
 * @elf_fd: ELF file descriptor
 * @encoding: file encoding
	...
	uint64_t sh_offset, char *string_table)
{
	...
	if (symbol.st_shndx == SHN_UNDEF)
		...
		printf("%0*lx ", ADDR_SIZE, symbol.st_value);

	/* Print symbol type code  */
	printf("%c ", get_symbol_type_64(elf_fd, encoding, sh_offset, &symbol));
...
/**
 * get_symbol_type_64 - returns letter representing a symbol's type
 * @elf_fd: ELF file descriptor
 * @encoding: file encoding
	...
	uint64_t sh_offset, Elf64_Sym *symbol)
{
	...
	return (get_symbol_type_from_section_64(elf_fd, encoding, sh_offset, symbol));
}

/**
 * get_symbol_type_from_section_64 - finds section referred by a symbol and
 ...
 * Return: type code
 **/
char get_symbol_type_from_section_64(
	int elf_fd, int encoding, uint64_t sh_offset, Elf64_Sym *symbol)
{
