...
/**
 * bswap_Elf64_Ehdr - MSB to LSB encoding for Elf64_Ehdr
 * @h: header
 **/
void bswap_Elf64_Ehdr(Elf64_Ehdr *h)
{
	...
	h->e_entry      = __builtin_bswap64(h->e_entry);
	h->e_phoff      = __builtin_bswap64(h->e_phoff);
	h->e_shoff      = __builtin_bswap64(h->e_shoff);
...
/**
 * bswap_Elf64_Shdr - MSB to LSB encoding for Elf64_Shdr
 * @h: header
 **/
void bswap_Elf64_Shdr(Elf64_Shdr *h)
{
	...
	h->sh_flags     = __builtin_bswap64(h->sh_flags);
	h->sh_addr      = __builtin_bswap64(h->sh_addr);
	h->sh_offset    = __builtin_bswap64(h->sh_offset);
	h->sh_size      = __builtin_bswap64(h->sh_size);
	h->sh_link      = __builtin_bswap32(h->sh_link);
	h->sh_info      = __builtin_bswap32(h->sh_info);
	h->sh_addralign = __builtin_bswap64(h->sh_addralign);
	h->sh_entsize   = __builtin_bswap64(h->sh_entsize);
}

/**
 * bswap_Elf64_Sym - MSB to LSB encoding for Elf64_Sym
 * @h: header
 **/
void bswap_Elf64_Sym(Elf64_Sym *h)
{
	h->st_name  = __builtin_bswap32(h->st_name);
	h->st_value = __builtin_bswap64(h->st_value);
	h->st_size  = __builtin_bswap64(h->st_size);
