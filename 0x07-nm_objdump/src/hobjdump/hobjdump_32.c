...
/**
 * print_section_stuff_32 - prints the section contents
 * @elf_fd: ELF file descriptor
...
void print_section_stuff_32(int elf_fd, Elf32_Ehdr *Ehdr, char *string_table)
{
	...
	for (i = 0; i < Ehdr->e_shnum; i++)
	{
		...
		{
			printf(" %0*x ", addr_size, Shdr.sh_addr + j);
...
/**
 * hobjdump_32 - hobjdump 32-bit case
 * @file: file to dump
...
int hobjdump_32(char *file, int elf_fd)
{
	...
	{
		printf("file format elf32-");
		printf("%s\n", Ehdr.e_ident[EI_DATA] == ELFDATA2MSB ? "big" : "little");
		printf("architecture: UNKNOWN!, ");
	}
	print_flags_32(elf_fd, &Ehdr);
	...
	print_section_stuff_32(elf_fd, &Ehdr, string_table);
	return (0);
}
